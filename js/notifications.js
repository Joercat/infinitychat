(function(){
  ...
  window.notifyNewMessage = function(message) {
    try {
      ...
      let title = (message && message.username) ? (message.username + " — new message") : "New message";
      let body = message && message.message_text ? stripHtml(message.message_text) : "";
  ...
  try {
    ...
    function wrapIfReady(){
      ...
      if (typeof window.renderMessage === "function") {
        let _orig = window.renderMessage;
        window.renderMessage = function(msg) {
          try { let res = _orig.apply(this, arguments); } catch(e) { let res = undefined; }
